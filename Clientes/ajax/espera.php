<?php 
require_once "../modelos/Espera.php";
...
switch ($_GET["op"]) {
    ...
    case 'listar':
		...
		case 'confirmado':
			...
			while ($reg=$rspta->fetch_object()) {
				$url='../vistas/recibo.php?id=';
				$data[]=array(
					"0"=>(($reg->estadoPago=='Pendiente'  )?  '<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->idpedido.')"><i class="fa fa-eye"></i></button>' : '<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->idpedido.')"><i class="fa fa-eye"></i></button>' .''.'<a href="'.$url.$reg->idpedido.'"> <button class="btn btn-info btn-xs"><i class="fa fa-file"></i></button></a>') ,
					"1"=> $reg->tienda ,
					"2"=> $reg->fechahora ,
					"3"=> $reg->tipo ,
				"4"=> (($reg->estadoPago!='Pagado')?'<span class="label bg-yellow"">Por Pagar</span>' : '<span class="label bg-green" >Pagado</span>' )   
			...
			case 'cancelado':
				...
				while ($reg=$rspta->fetch_object()) {
					...
					$data[]=array(
						...
						"2"=> (($reg->estado=='Cancelado')?'<span class="label bg-red"">Cancelado</span>' : '<span class="label bg-green" >Pagado</span>' )   
