<?php 
require_once "../modelos/Pedidosentregados.php";
...
switch ($_GET["op"]) {
    ...
    case 'listar':
		...
		while ($reg=$rspta->fetch_object()) {
			...
			"6"=>'<span class="label bg-green">'.$reg->estado.'</span>',
			"7"=>(($reg->estado=='Confirmado' or $reg->estado!=='Confirmado' )?  '<button class="btn btn-danger btn-xs" onclick="desactivar('.$reg->idpedido.')"><i class="fa fa-close"></i></button>' : '<button class="btn btn-warning btn-xs" onclick="desactivar('.$reg->idpedido.')"><i class="fa fa-music"></i></button>'  )
              );
		}
		$results=array(
