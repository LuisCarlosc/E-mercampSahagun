...
if (!isset($_SESSION['nombre']) AND $_SESSION['nombre'] != 1) {
	...
	if (empty($_POST['claveactual'])) {
		...
		$sql2="SELECT clave from users where  iduser= '".$usuario."' and  idtienda=".$_SESSION['idtienda'];
        $query2=mysqli_query($conexion,$sql2);
        $row=mysqli_fetch_array($query2);

		if($row['clave'] == $clavehash1){
		 
			$sql="UPDATE users SET clave='".$clavehash2."' WHERE iduser='".$usuario."' and idtienda= ".$_SESSION['idtienda'];
			$query_update = mysqli_query($conexion,$sql);
				if ($query_update){
					$messages[] = "La contraseña han sido actualizadas satisfactoriamente.";
				} else{
